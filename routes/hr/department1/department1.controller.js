...
const createSingleDepartment = async (req, res) => {
  if (req.query.query === "deletemany") {
    try {
      ...
      const deletedDepartment = await prisma.department1.deleteMany({
        where: {
          id: {
            in: req.body,
          },
    ...
    try {
      ...
      const createdDepartment = await prisma.department1.createMany({
        data: req.body,
        skipDuplicates: true,
      });
    ...
    try {
      const createdDepartment = await prisma.department1.create({
        data: {
          name: req.body.name,
        },
      });
...
const getAllDepartment = async (req, res) => {
  if (req.query.query === "all") {
    const allDepartment = await prisma.department1.findMany({
      include: {
        user: {
          select: {
            id: true,
...
const getSingleDepartment = async (req, res) => {
  try {
    const singleDepartment = await prisma.department1.findUnique({
      where: {
...
const updateSingleDepartment = async (req, res) => {
  try {
    const updatedDepartment = await prisma.department1.update({
      where: {
        id: Number(req.params.id),
      },
      data: {
...
const deletedDepartment = async (req, res) => {
  try {
    const deletedDepartment = await prisma.department1.update({
      where: {
        id: Number(req.params.id),
      },
      data: {
